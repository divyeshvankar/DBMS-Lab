...
void bubbleSort(int n)
{
    ...
    for (int i = 0; i < n - 1; i++)
    {
        ...
        {
            if (strcmp(grades[j + 1].name, grades[j].name) < 0)
            {
                struct gra temp = grades[j+1];
                grades[j+1] = grades[j];
                grades[j] = temp;
                c++;
            }
            else if (strcmp(grades[j + 1].name, grades[j].name) == 0)
            {
			if(grades[j+1].credits > grades[j].credits)
                {
                    struct gra temp = grades[j+1];
                    grades[j+1] = grades[j];
                    grades[j] = temp;
		...
		else if (grades[j+1].credits == grades[j].credits)
                {
                    if (strcmp(grades[j + 1].grade, grades[j].grade) <= 0)
                    {
                        struct gra temp = grades[j+1];
                        grades[j+1] = grades[j];
                        grades[j] = temp;
...
int main()
{
    ...
    FILE *fout = NULL;
    int n = 0;
    /*fin = fopen("students01.csv", "a+");
	fprintf(fin,"\n");
	fclose(fin);
	fin = fopen("grades01.csv", "a+");
	fprintf(fin,"\n");
	fclose(fin);
	fin = fopen("courses01.csv", "a+");
	fprintf(fin,"\n");
	fclose(fin);*/
    ...
    for (int z = 0; z < Ccount; z++)
    {
        for (int k = 0; k < Gcount; k++)
        {
            ...
            {
                strcpy(grades[k].course_name, courses[z].title);  grades[k].credits = courses[z].C;
            }
        }
    }
    bubbleSort(Gcount);
    fout = fopen("grades-sorted-B.csv", "w");
